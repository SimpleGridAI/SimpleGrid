const ROLES = [
  {
    ...
    what: [
      'Own US pipeline end-to-end: ICP definition, outbound, partnerships, pilots, close.',
      'Personally lead the first 5-10 US deals alongside the founder. Discovery, scoping, pricing, negotiation.',
      'Stand up a repeatable motion: messaging, sequences, demo flow, pricing tiers, contract templates.',
      'Build channels: industry associations (NAM, AME, regional manufacturing councils), MSPs, fractional-CFO networks, accounting partners.',
      'Hire the first AE / SDR when the motion is proven, not before.',
    ],
    you: [
      '15+ years in B2B SaaS GTM, with at least one CRO or VP Sales seat at a Series A-C company.',
      'Has personally closed mid-market manufacturing, supply chain, or vertical-SaaS deals in the US.',
      'Strong existing network among mid-market manufacturers, COOs, plant managers, and industry consultants.',
      'Operator-friendly. You can hold your own in a conversation about WIP, MOQs, and AP/AR.',
      'Comfortable being the most senior US person at the company on day one.',
    ],
    not: [
      'A pure-strategy role. You will be on calls, on slides, and on planes.',
      'A passive board seat. Fractional means part-time, not hands-off.',
    ],
    process: [
      'Founder call.',
      'Working session: you walk us through how you would run our first US quarter.',
      'References from past founder partners.',
      'Engagement letter, 90-day evaluation, then long-term.',
    ],
  },
  {
    ...
    what: [
      'Customer ops: own followups, deployment trackers, and customer-success notes for every live account.',
      'Sales ops: research target accounts, draft outbound, prep founder for calls, log everything in CRM.',
      'Content: turn deployment learnings into case studies, blog posts, and product docs.',
      'Internal tooling: keep dashboards, hiring trackers, and finance trackers honest.',
      'Anything that is on fire that nobody else is owning.',
    ],
    you: [
      'Final-year undergrad, recent grad, or pre-MBA / pre-grad-school.',
      'Strong written English. We write everything down here.',
      'Ridiculously organized. Things you own do not slip.',
      'Comfortable with ambiguity, allergic to fluff, fast on a keyboard.',
      'Bonus: prior startup internship, debate / writing background, basic SQL or spreadsheets-at-depth.',
    ],
    not: [
      'A remote internship. We want you in the room.',
      'A passive role. We will not assign you a project; you will find the next thing yourself.',
    ],
    process: [
      'A short written application (3 questions, ~30 min).',
      'A 30-min call with the founder.',
      'A 1-day paid trial in the office.',
      'Offer.',
    ],
  },
];
...
function WhyThis() {
  return (
    <section className="section">
      <div className="container">
        <Reveal>
          <div className="tag">WHY THIS</div>
          <h2 className="h2">A few things to know before you apply.</h2>
        </Reveal>
          ...
          {[
            ...
            <Reveal key={i} delay={i * 100}>
              <div style={{background:'#fff',border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',padding:24,height:'100%'}}>
                <h3 style={{fontFamily:'var(--font-heading)',fontSize:18,fontWeight:700,margin:'0 0 10px',color:'var(--fg1)',letterSpacing:'-0.01em'}}>{c.t}</h3>
                <p style={{fontSize:14,color:'var(--fg2)',lineHeight:1.6,margin:0}}>{c.b}</p>
              </div>
            </Reveal>
          ))}
        </div>
...
function RoleSummary({ r, onOpen }) {
  return (
    <div style={{background:'#fff',border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',padding:'22px 26px',display:'grid',gridTemplateColumns:'160px 1fr auto',gap:24,alignItems:'center'}} className="role-row">
      <div>
        <div style={{fontSize:10,textTransform:'uppercase',letterSpacing:'0.12em',color:'var(--fg3)',fontWeight:700,marginBottom:4}}>{r.team}</div>
        <div style={{fontSize:12,color:'var(--fg3)'}}>{r.type}</div>
      </div>
      <div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:8,alignItems:'stretch'}}>
        <button onClick={() => onOpen(r.id)} className="btn btn-sm btn-secondary" style={{whiteSpace:'nowrap'}}>Read full JD →</button>
        <a href={'mailto:hello@simplegrid.ai?subject=Application:%20' + encodeURIComponent(r.t)} className="btn btn-sm btn-primary" style={{whiteSpace:'nowrap'}}>Apply</a>
      </div>
    </div>
  );
}
...
function RoleDetail({ r }) {
  ...
  return (
    <div id={r.id} style={{background:'#fff',border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',padding:'40px 44px',scrollMarginTop:80}}>
      ...
      <Sec title="What we are looking for" items={r.you} />
      <Sec title="What this is not" items={r.not} color="var(--sg-red)" />
      <Sec title="Process" items={r.process} color="var(--sg-green)" />
      <div style={{marginTop:32,paddingTop:24,borderTop:'1px solid var(--border)',display:'flex',justifyContent:'space-between',alignItems:'center',gap:16,flexWrap:'wrap'}}>
        <div style={{fontSize:14,color:'var(--fg2)'}}>Apply by emailing <a href={'mailto:hello@simplegrid.ai?subject=Application:%20' + encodeURIComponent(r.t)} style={{color:'var(--sg-blue)',fontWeight:600}}>hello@simplegrid.ai</a> with your CV or LinkedIn and a one-paragraph note on why this.</div>
        <a href={'mailto:hello@simplegrid.ai?subject=Application:%20' + encodeURIComponent(r.t)} className="btn btn-primary">Apply for this role</a>
      </div>
    </div>
  );
}
  ...
  React.useEffect(() => {
    ...
    ld.textContent = JSON.stringify({
      ...
      '@graph': ROLES.map(r => {
        ...
        const through = new Date(new Date(posted + 'T00:00:00Z').getTime() + 90 * 24 * 60 * 60 * 1000)
        ...
        return {
          ...
          'hiringOrganization': { '@type': 'Organization', '@id': 'https://simplegrid.ai/#org', 'name': 'SimpleGrid', 'sameAs': 'https://simplegrid.ai/' },
          'jobLocation': { '@type': 'Place', 'address': { '@type': 'PostalAddress', 'addressLocality': r.loc.includes('Bangalore') ? 'Bengaluru' : (r.loc.includes('Remote') ? 'Remote' : r.loc), 'addressCountry': r.loc.includes('US') ? 'US' : 'IN' } },
          'datePosted': posted,
          'validThrough': through,
          'directApply': true,
          'identifier': { '@type': 'PropertyValue', 'name': 'SimpleGrid', 'value': r.id }
        };
      })
    });
  ...
  return (<>
    ...
    <section className="section section-alt" id="roles">
      <div className="container">
        <Reveal>
          ...
          <h2 className="h2">Where we need help.</h2>
        </Reveal>
        <div style={{marginTop:28,display:'flex',flexDirection:'column',gap:12}}>
          {ROLES.map((r,i) => (
            <Reveal key={r.id} delay={i * 60}>
              <RoleSummary r={r} onOpen={handleOpen} />
            </Reveal>
          ))}
        </div>
      </div>
    ...
    <section className="section">
      <div className="container">
        ...
        <Reveal delay={200}>
          <p style={{fontSize:14,color:'var(--fg2)',marginTop:32,textAlign:'center'}}>
            Don't see your role? Email <a href="mailto:hello@simplegrid.ai" style={{color:'var(--sg-blue)'}}>hello@simplegrid.ai</a> with how you'd contribute.
          </p>
        </Reveal>
      </div>
    </section>
...
ReactDOM.createRoot(document.getElementById('root')).render(<HiringPage />);
