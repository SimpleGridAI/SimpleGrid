        ...
        <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 1100, padding: '0 32px' }}>
            ...
            {stages.map((s, i) => {
              ...
              return (
                <React.Fragment key={s.id}>
                  ...
                  {/* Connector line */}
                  {i < stages.length - 1 && (
                    <div style={{ width: 24, height: 2, flexShrink: 0, position: 'relative', top: -12 }}>
                      <div style={{ position: 'absolute', inset: 0, background: '#E5E8ED', borderRadius: 1 }}></div>
                      <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              {/* Animated factory visual */}
              <FactoryScene stage={activeStage} progress={stageProgress} color={stages[activeStage].color} />
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <div style={{
                ...
                letterSpacing: '0.14em', color: stages[activeStage].color, marginBottom: 8,
              }}>Step {activeStage + 1} of {stages.length}</div>
              <h3 style={{
              ...
              <div style={{
                background: 'var(--sg-off-white)', borderRadius: 8, padding: 14,
                fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg2)', lineHeight: 1.7,
              }}>
                <div style={{ color: stages[activeStage].color, fontWeight: 600 }}>
                  Event: {stages[activeStage].label.toUpperCase()}
                </div>
                <div>Entity: SO-4521 → Stage {activeStage + 1}</div>
                <div>Actor: System · {new Date().toLocaleTimeString()}</div>
                <div style={{ color: 'var(--sg-green)' }}>Status: ● Recorded</div>
              </div>
            </div>
          </div>
...
function FactoryScene({ stage, progress, color }) {
  ...
  const scenes = [
    ...
    <svg key="0" viewBox="0 0 300 200" style={{width:'100%'}} aria-hidden="true">
      ...
      <line x1="90" y1="110" x2="200" y2="110" stroke="#E5E8ED" strokeWidth="2"/>
      <line x1="90" y1="130" x2="160" y2="130" stroke="#E5E8ED" strokeWidth="2"/>
      <text x="150" y="56" textAnchor="middle" fill={color} fontSize="11" fontWeight="700" fontFamily="var(--font-heading)">PURCHASE ORDER</text>
      <circle cx="240" cy="60" r="16" fill={color} opacity="0.15"/>
      <text x="240" y="64" textAnchor="middle" fill={color} fontSize="14" fontWeight="700">AI</text>
    ...
    <svg key="2" viewBox="0 0 300 200" style={{width:'100%'}} aria-hidden="true">
      {[0,1,2,3].map(i => {
        ...
        return <g key={i}>
          <rect x={x} y={arrived ? 80 : 20-progress*60} width="55" height="55" rx="6" fill={arrived ? color+'22' : '#FAFBFC'} stroke={arrived ? color : '#E5E8ED'} strokeWidth="1.5" style={{transition:'all 0.5s cubic-bezier(0.2,0.8,0.2,1)'}}/>
          <text x={x+27} y={arrived ? 112 : 52-progress*60} textAnchor="middle" fill={arrived ? color : '#9CA3AF'} fontSize="18" style={{transition:'all 0.5s'}}>📦</text>
        </g>;
      })}
      <line x1="20" y1="145" x2="280" y2="145" stroke="#E5E8ED" strokeWidth="2"/>
    ...
    <svg key="3" viewBox="0 0 300 200" style={{width:'100%'}} aria-hidden="true">
      ...
      {[0,1,2,3,4,5].map(i => {
        ...
        return <rect key={i} x={75+col*55} y={65+row*45} width="45" height="35" rx="4" fill={filled ? color+'22' : '#fff'} stroke={filled ? color : '#E5E8ED'} strokeWidth="1" style={{transition:'all 0.4s', transitionDelay: i*80+'ms'}}/>;
      })}
    ...
    <svg key="4" viewBox="0 0 300 200" style={{width:'100%'}} aria-hidden="true">
      ...
      {[0,1,2,3,4].map(i => {
        ...
        return <g key={i}>
          <rect x={x} y={beltY-30} width="35" height="25" rx="4" fill={color+'22'} stroke={color} strokeWidth="1"/>
          <text x={x+17} y={beltY-13} textAnchor="middle" fontSize="12">⚙️</text>
        </g>;
      })}
      <line x1="20" y1={beltY+15} x2="280" y2={beltY+15} stroke="none"/>
      {['Machining','Assembly','Sanding','Finishing'].map((s,i) => (
        <text key={i} x={50+i*60} y={beltY+40} textAnchor="middle" fill="var(--fg3)" fontSize="8">{s}</text>
      ))}
    ...
    <svg key="5" viewBox="0 0 300 200" style={{width:'100%'}} aria-hidden="true">
      {[0,1,2,3,4,5].map(i => {
        ...
        return <g key={i}>
          <rect x={50+col*75} y={40+row*65} width="60" height="50" rx="6" fill={failed ? '#FEE2E2' : passed ? '#D1FAE5' : '#FAFBFC'} stroke={failed ? '#EF4444' : passed ? '#10B981' : '#E5E8ED'} strokeWidth="1.5" style={{transition:'all 0.3s'}}/>
          <text x={80+col*75} y={72+row*65} textAnchor="middle" fontSize="16">{failed ? '✕' : passed ? '✓' : '?'}</text>
        </g>;
      })}
    ...
    <svg key="6" viewBox="0 0 300 200" style={{width:'100%'}} aria-hidden="true">
      {[0,1,2].map(i => {
        ...
        return <g key={i}>
          <rect x={x} y="50" width="70" height="70" rx="6" fill={sealed ? color+'15' : '#FAFBFC'} stroke={sealed ? color : '#E5E8ED'} strokeWidth="1.5" style={{transition:'all 0.4s'}}/>
          {sealed && <line x1={x+15} y1="85" x2={x+55} y2="85" stroke={color} strokeWidth="2" strokeDasharray="none"/>}
          {sealed && <line x1={x+35} y1="60" x2={x+35} y2="110" stroke={color} strokeWidth="2"/>}
          <text x={x+35} y="140" textAnchor="middle" fill="var(--fg3)" fontSize="9">Box {i+1}</text>
        </g>;
      })}
    ...
    <svg key="7" viewBox="0 0 300 200" style={{width:'100%'}} aria-hidden="true">
      <g style={{transform: `translateX(${-80 + progress * 200}px)`, transition: 'transform 0.3s linear'}}>
        ...
        <circle cx="100" cy="140" r="12" fill="#fff" stroke={color} strokeWidth="2"/>
        <circle cx="160" cy="140" r="12" fill="#fff" stroke={color} strokeWidth="2"/>
        <circle cx="210" cy="140" r="12" fill="#fff" stroke={color} strokeWidth="2"/>
        <rect x="185" y="92" width="18" height="14" rx="2" fill="#fff" opacity="0.6"/>
      </g>
      <line x1="0" y1="152" x2="300" y2="152" stroke="#E5E8ED" strokeWidth="2"/>
      <text x="150" y="185" textAnchor="middle" fill={progress > 0.8 ? 'var(--sg-green)' : 'var(--fg3)'} fontSize="10" fontWeight={progress > 0.8 ? 700 : 400}>
