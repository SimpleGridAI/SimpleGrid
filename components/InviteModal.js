...
function InviteModal({
  ...
  React.useEffect(() => {
    ...
    const onKey = e => {
      ...
      if (!overlay) return;
      const focusables = overlay.querySelectorAll('a[href],button:not([disabled]),input:not([disabled]),textarea:not([disabled]),select:not([disabled]),[tabindex]:not([tabindex="-1"])');
  ...
  const submit = async e => {
    ...
    if (window.sgTrack) window.sgTrack('invite_modal_submitted', {
      form: 'invite'
    });
    try {
      ...
      if (res.ok && (json.success === 'true' || json.success === true)) {
        setState('success');
        if (window.sgTrack) window.sgTrack('invite_modal_succeeded', {
          form: 'invite'
        });
      } else {
  ...
  if (state === 'success') {
    return /*#__PURE__*/React.createElement("div", {
      ...
      role: "dialog",
      "aria-modal": "true",
      "aria-labelledby": "invite-success-h",
      style: {
        position: 'relative',
        textAlign: 'center'
      }
    ...
    }, "Thanks. We'll review within 48 hours."), /*#__PURE__*/React.createElement("a", {
      href: "https://cal.com/simplegrid-ai",
      target: "_blank",
      rel: "noopener noreferrer",
      "data-cta": "invite_success",
    ...
    }, "Skip the wait - book a call now"), /*#__PURE__*/React.createElement("button", {
      type: "button",
  ...
  }, state === 'submitting' ? 'Sending…' : 'Book a demo →'), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 11,
      color: 'var(--fg3)',
      textAlign: 'center',
  ...
  }, "We respond within 48 hours \xB7 Your data is used only for your deployment - see ", /*#__PURE__*/React.createElement("a", {
    href: "/privacy.html",
    style: {
      color: 'var(--sg-blue)'
    }
